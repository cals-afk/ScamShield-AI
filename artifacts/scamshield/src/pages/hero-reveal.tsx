...
export default function HeroReveal() {
  ...
  return (
    ...
    >
      ...
      {/* Hero photo — fades in when background fetch completes */}
      {heroImageUrl && (
        <div
      ...
      {/* Top-left label */}
      <div className="absolute top-4 sm:top-6 left-4 sm:left-6 pointer-events-none animate-slide-left-in delay-800">
        <p className="font-mono text-[10px] sm:text-xs tracking-[0.3em] sm:tracking-[0.4em] uppercase" style={{ color: `${primary}88` }}>
          Identity Confirmed
        </p>
      </div>

      {/* Top-right label */}
      <div className="absolute top-4 sm:top-6 right-4 sm:right-6 pointer-events-none flex items-center gap-1.5 sm:gap-2 animate-slide-right-in delay-900">
        <div
          className="w-1.5 h-1.5 rounded-full animate-ping"
          style={{ backgroundColor: primary, animationDuration: "1.4s" }}
        />
        <p className="font-mono text-[10px] sm:text-xs tracking-[0.25em] sm:tracking-[0.3em] uppercase" style={{ color: `${primary}99` }}>
          {theme?.label ?? "SENTINEL MODE"}
        </p>
      </div>

      {/* Bottom: mode name + tagline */}
      <div className="absolute bottom-0 left-0 right-0 px-4 sm:px-8 pb-6 sm:pb-10 pointer-events-none">
        <div className="animate-hero-name-rise">
          <div className="w-10 sm:w-16 h-px mb-3 sm:mb-5" style={{ backgroundColor: `${primary}88` }} />
          <h1
            className="text-3xl sm:text-5xl md:text-6xl font-black tracking-wider uppercase leading-none mb-2 sm:mb-3"
            style={{
              color: "#ffffff",
          ...
          <p
            className="font-mono text-[10px] sm:text-sm tracking-[0.2em] sm:tracking-[0.25em] uppercase"
